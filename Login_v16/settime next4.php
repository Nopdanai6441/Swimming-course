...
<body>
	...
	if (isset($_POST['Submit'])) {
		$dateTime = array();
		$id = $_GET['id'];
			 $date = $_POST['day4'].$_POST['datee'];
			 $date = date('Y-m-d H:i:s', StrToTime($date));
			 $sql = "INSERT INTO `memnext` (`userid`, `namenext`, `phone`, `timenext`, `numm`) VALUES ('".$_GET['id']."', '" . $_POST['name'] . "', '" . $_POST['phone'] . "', '" .$date. "', '4')";
...
<?php while ($row = mysqli_fetch_array($result)) {?>
					...
					<div class="wrap-input100 validate-input" data-validate="กรุณากรอกข้อมูล">
						<?php $datea=date_format(new datetime ($row["day4"]),"d-m-Y")  ?>
						<input class="input100" type="text" name="day4" value="<?=$datea?>">
