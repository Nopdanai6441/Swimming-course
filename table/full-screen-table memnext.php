...
<body>
  ...
  <div class="container">
    ...
    echo  "<th>เบอร์โทร</th>";
    echo  "<th>ครั้งที่เรียน</th>";
    echo  "<th>วันที่ขอเลื่อน</th>";
    ...
    while ($row = mysqli_fetch_array($result)) {
      echo "<tr>";
      echo "<td>" . $row["namenext"] . "</td>";
      echo "<td>" . $row["phone"] . "</td>";
      echo "<td>" . $row["numm"] . "</td>";
      if($row["timenext"]=='0000-00-00 00:00:00'){echo "<td> 00-00-0000 00:00 <br></td>";}else{echo "<td>" .date_format(new datetime ($row["timenext"]),"d-m-Y H:i"). "<br></td>";}
      echo "<td><a href='../Login_v16/settime in1.php?id=$row[id]&namen=$row[namenext]&idu=$row[userid]&nummx=$row[numm]' class='btn btn-secondary btn-sm' > เลื่อนนัด </a></td>";
      echo "<td><a href='del next.php?id=$row[id]' class='btn btn-secondary btn-sm' > ลบ </a></td>";
      echo "</tr>";
      }
    echo "</tbody>";
    echo "</table>";
